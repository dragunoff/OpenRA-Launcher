...
void main() {
  ...
  final tReleaseResponse =
      TestUtils.getJsonStringFromFile('github_json/mod-database-release.json');
  final tDatabaseResponse =
      TestUtils.getJsonStringFromFile('github_json/mod-database.json');

  final tAssetUrl =
      'https://github.com/dragunoff/OpenRA-Mod-Database/releases/download/database-2026-01-01/database.json';
  ...
  group('ModDatabaseDataSourceImpl', () {
    group('getModDatabase', () {
      test('should fetch the release endpoint and the database asset',
          () async {
        // given
        when(mockClient.read(tReleaseEndpoint, headers: anyNamed('headers')))
      ...
      test('should return a ServerFailure when database asset is missing',
          () async {
        // given
      ...
      test('should return an empty database when the database is empty',
          () async {
        // given
        when(mockClient.read(tReleaseEndpoint, headers: anyNamed('headers')))
            .thenAnswer((_) async => tReleaseResponse);
        when(mockClient.read(Uri.parse(tAssetUrl), headers: anyNamed('headers')))
            .thenAnswer((_) async => '{}');
